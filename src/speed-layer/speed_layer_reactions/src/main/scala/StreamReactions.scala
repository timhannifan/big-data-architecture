...
object StreamReactions {
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("StreamReactions")
    val ssc = new StreamingContext(sparkConf, Seconds(2))

    // Create direct kafka stream with brokers and topics
    val topicsSet = Set("hannifan_latest_interactions")
    // Create direct kafka stream with brokers and topics
    ...
    val reports = serializedRecords.map(rec => mapper.readValue(rec, classOf[ReactionReport]))
